...
describe('AuthModule (e2e) - google', () => {
    ...
    describe('POST /api/auth/google', () => {
        it('처음 구글 ID 연동', async () => {
            const res = await request(app.getHttpServer()).post('/api/auth/google').send({ token: 'google_token' }).expect(HttpStatus.CREATED);
        ...
        it('구글 ID 연동만 한 경우, user_social 테이블은 기존꺼를 활용', async () => {
            const res = await request(app.getHttpServer()).post('/api/auth/google').send({ token: 'google_token' }).expect(HttpStatus.CREATED);
